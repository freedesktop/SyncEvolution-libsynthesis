...
///
///   - fLastItemStatus   = status code (uInt16) of last item
      ...
      /// For datastores that can resume in middle of a chunked item (fConfigP->fResumeItemSupport==true):
        ...
        if (dsResumeChunkedSupportedInDB()) {
          if (strucmp(fieldname.c_str(),"partialitemstate")==0) {
            usP = (uInt16*)&fPartialItemState;  // enum
          }
          else if (strucmp(fieldname.c_str(),"lastitemstatus")==0) {
            usP= &fLastItemStatus; // status code (uInt16) of last item
          }
...
/// - fAbortStatusCode                  : uInt16, if == 0, sync with this datastore was ok, if <>0, there was an error.
/// - fSessionP->getAbortReasonStatus() : uInt16, shows status of entire session at the point when datastore finishes syncing.

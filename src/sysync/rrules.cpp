  ...
  if (es.freq=='D') {
    ...
    // found occurrence
      ...
      if (es.firstmask==0 && es.lastmask==0) {
		  	if (es.freqmod=='W') {
          es.firstmask = (uInt64)1<<(es.cursorWDay+DaysPerWk*((es.cursorWDay-1)/DaysPerWk)); // set start weekday in mask
        }
        else {
      ...
      if (es.expansionStartDayOffset) {
        ...
        // startYear/month/day now on first candidate
        // - calculate linear start
        es.cursor = date2lineartime(es.startyear, es.startmonth, 1);
        es.cursorMLen = getMonthDays(lineartime2dateonly(es.cursor));

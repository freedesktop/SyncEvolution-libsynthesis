...
bool TSmlCommand::handleStatus(TStatusCommand *aStatusCmdP)
{
  ...
  uInt16 statuscode = aStatusCmdP->getStatusCode();
  if (statuscode<200) {
    // informational
    switch (statuscode) {
      case 101:
...
TStatusCommand *TSmlCommand::newStatusCommand(uInt16 aStatusCode, const char *aStringItem)
{
...
void TSmlCommand::issueStatusCommand(uInt16 aStatusCode)
{
...
TUnimplementedCommand::TUnimplementedCommand(
  ...
  uInt16 aStatusCode                 // status code to be returned on execution
) :
...
// set Status code
void TStatusCommand::setStatusCode(uInt16 aStatusCode)
{
  fStatusCode = aStatusCode;

...
void lineartime2tm(lineartime_t aLt, struct_tm *aTm);
// convert struct tm to linear time (in milliseconds)
lineartime_t tm2lineartime(struct_tm *aTm);


// get system current date/time in local (!) linear time
lineartime_t localNowAsLineartime(void);
// get system current date/time in UTC (!) linear time
lineartime_t utcNowAsLineartime(void);


// get offset from UTC to local time (UTC+offset = local time)
// relative to the given reference UTC timestamp (NULL if using current date/time)
sInt32 zoneOffsetAsSeconds(lineartime_t aUTCReference);
lineartime_t zoneOffsetAsLineartime(lineartime_t aUTCReference);

// make local timestamp from UTC timestamp (must use date from
// timestamp to determine if local time has daylight savings or not)
lineartime_t makeLocalTimestamp(lineartime_t aUTCTimestamp);
// make UTC timestamp from local timestamp (must use date from
// timestamp to determine if local time had daylight savings or not)
lineartime_t makeUTCTimestamp(lineartime_t aLocalTimestamp);

...
{
  sInt16 mo,h,mi;
  lineartime2date(tim, NULL, &mo, NULL);
  lineartime2time(tim, &h, &mi, NULL, NULL);
...
static void AdaptDay( lineartime_t &tim, tChange c, bool noCalc )
{
  sInt16 y,mo,h,m,dowk;

  lineartime2date( tim, &y,&mo,NULL );
  lineartime2time( tim, &h,&m, NULL,NULL );

  tim  = date2lineartime(y,mo,1); // make calculation with the 1st of month
  dowk = lineartime2weekday(tim); // get weekday

  if (noCalc) return;

  sInt16 d= c.wDayOfWeek - dowk + 1;
  if (d<1)   d+= DaysOfWeek; // this is the 1st occurance of the given weekday
  d+= (c.wNth-1)*DaysOfWeek;

  AdjustDay  ( d, mo, y ); // and ajust <d> within the month
  tim  = date2lineartime( y,mo,d ) + time2lineartime( h,m,0,0 ); // don't forget time
} // AdaptDay
...
static void tChange_to_Rtm( tChange c, int year, RType &r, lineartime_t &tim )
{
  ...
  tim = date2lineartime(year,c.wMonth,1) + time2lineartime(c.wHour, c.wMinute, 0, 0);
  AdaptDay( tim, c, noCalc ); // get wDayOfWeek/wNth, fitting for <year>
...
/*! Fill in the TZ info */
static bool GetTZInfo( const char*   aText,
                       const char*   aIdent, tChange &c, short &cBias, string &cName,
                       GZones *g,
                       TDebugLogger* aLogP )
{
  RType     r;
  ...
  string             a= VStr( aText, aIdent );
  string rr= VValue( a, VTZ_RR    ); // sub items: - RRULE
  string st= VValue( a, VTZ_START ); //            - start time
...
// convert to UTC => %% corrected: has nothing to do with UTC here (bfo)
//TzConvertTimestamp (                   dtstart, tctx, TCTX_UTC, g, TCTX_SYSTEM);
  if (!RRULE2toInternalR   ( rr.c_str(), dtstart,       r, aLogP   )) return false;
  if (!Get_Bias( of,ot, cBias ))                                      return false;

  string vvv;
  //bool ok= //%%% not used at this time
                          ...
                          string*        aTzidP )  ///< if not NULL, receives TZID as found in VTIMEZONE
{
  ...
  timecontext_t lContext;
                            ...
                            t.ident  = "";
                            t.dynYear= "CUR";
  GetTZInfo( aText,VTZ_STD, t.std, t.bias, stdName, g, aLogP );
  GetTZInfo( aText,VTZ_DST, t.dst, biasD,  dstName, g, aLogP );

	// get TZID as found in VTIMEZONE
  string  tName= VValue( aText, VTZ_ID );
  if (aTzidP) *aTzidP = tName; // return the original TZID as found, needed to match with TZID occurences in rest of vCalendar
  ...
  bool ok= true;
  if   (t.bias==biasD) ClrDST( t ); // no DST ?
  else {
  ...
  string          tzName;
  ok= FoundTZ( t, tzName, aContext, g );

  #ifdef SYSYNC_TOOL
    LOGDEBUGPRINTFX( aLogP, DBG_PARSE,( "tzName='%s' tx=%08X %d %s",
                                         tzName.c_str(),   aContext,
                                         TCTX_OFFSCONTEXT( aContext ), Bo( ok ) ) );
  #endif

  t.name= tName.c_str();

  if (!ok) ok= FoundTZ( t, tzName, aContext, g, true );

  if  (ok && t.std.wMonth!=0 &&
             t.dst.wMonth!=0) {
             t.name = stdName.c_str();
             t.ident= "s"; // standard
    FoundTZ( t, lName,lContext, g, true );

             t.name = dstName.c_str();
             t.ident= "d"; // daylight saving
             t.bias = biasD;
    FoundTZ( t, lName,lContext, g, true );
  } // if
  ...
  /*
  timecontext_t searchOffs= 0;
  while (true) {
    string            tzName= "";
    if (!FoundTZ( tz, tzName, aContext, searchOffs )) break;

    printf( "tzName='%s' tx=%08X %d\n", tzName.c_str(),
             aContext, TCTX_OFFSCONTEXT( aContext ) );
  //searchOffs= TCTX_OFFSCONTEXT( aContext );
    searchOffs= aContext;
  } // while
  */

  /*
  string aStd= VItem( aText,VTZ_STD );
  string aDlt= VItem( aText,VTZ_DST );
  printf("\n STD='%s'\n DLT='%s'\n\n", aStd.c_str(),aDlt.c_str() );

  string rStd= LItem( aStd, VTZ_RR );
  string rDlt= LItem( aDlt, VTZ_RR );
  printf(  "rSTD='%s'\nrDLT='%s'\n\n", rStd.c_str(),rDlt.c_str() );

  string dStd= LItem( aStd, VTZ_START );
  string dDlt= LItem( aDlt, VTZ_START );
  printf(  "dSTD='%s'\ndDLT='%s'\n\n", dStd.c_str(),dDlt.c_str() );

  lineartime_t dststart;
  if (!ISO8601ToTimeStamp( dStd.c_str(), dststart )) return false;
  lineartime_t ddtstart;
  if (!ISO8601ToTimeStamp( dDlt.c_str(), ddtstart )) return false;

  RType r;
  bool ok= RRULE2toInternalR( rStd.c_str(), dststart, r, aLogP );
       ok= RRULE2toInternalR( rDlt.c_str(), ddtstart, r, aLogP );

//const char* s= "FREQ=MONTHLY;BYDAY=-3SU";
//const char* s= "FREQ=YEARLY;BYMONTH=4,5,6,8";
//const char* s= "FREQ=YEARLY;BYMONTH=4";
//const char* s= "FREQ=YEARLY;BYDAY=1MO,2TU";
  const char* s= "FREQ=MONTHLY;INTERVAL=12;BYDAY=TH,1SU,2MO;UNTIL=20050401";
//const char* s= "FREQ=MONTHLY;INTERVAL=12;BYDAY=TH;UNTIL=20050401";
//RRULE2toInternalR( s, ddtstart, r, aLogP );
                          ...
                          sInt32         testYear )
{
  ...
  bool withDST=    t.std.wMonth!=0 &&
                   t.dst.wMonth!=0;
  if      (strcmp( t.ident,"$" )==0) { aText= t.name; return true; } // just give it back
  ...
  if      (strcmp( t.dynYear,"")!=0) {
    timecontext_t aDJ= aContext+1;
    GetTZ       ( aDJ, tp, g, -1 );

    // but only if not the first entry of dynYear
    if    (strcmp( t.name,   tp.name    )==0 &&
           strcmp( t.dynYear,tp.dynYear )!=0) {
      y_std= atoi( t.dynYear );
      y_dst= yy;
    } // if
  } // if

  // at least one STANDARD or DAYLIGHT info is mandatory
  aText=    Property      (    VTZ_ID,      t.name ) +
...
static string PeeledStr( string aStr, string bv, string ev )
{
...
string  VStr( string aStr, string value ) {

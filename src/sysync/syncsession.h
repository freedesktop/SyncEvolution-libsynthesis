...
public:
  ...
  uInt16 fRejectStatusCode; // if >=0, attempt to connect will always be rejected with given status code
...
public:
  ...
  // Abort session
  void AbortSession(uInt16 aStatusCode, bool aLocalProblem, uInt16 aReason=0); // resets session and sets aborted flag to prevent further processing of message
  // Suspend session
  void SuspendSession(uInt16 aReason);
  // Session status
  ...
  void DatastoreFailed(uInt16 aStatusCode, bool aLocalProblem=false); // let session know that datastore has failed
  ...
  void AbortCommandProcessing(uInt16 aStatusCode); // all further commands in message will be answered with given status
  ...
  virtual bool handleAlertStatus(
    uInt16 aStatusCode,  // status code
...
protected:
  ...
  uInt16 fAbortReasonStatus; // if fAborted, this contains a status code what command has aborted the session
  ...
  uInt16 fStatusCodeForIgnored; // if fIgnoreIncomingCommands is set, this status code will be used to reply all incoming commands

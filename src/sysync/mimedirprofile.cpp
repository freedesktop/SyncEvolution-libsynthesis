  ...
  switch (aConvDefP->convmode) {
    ...
    case CONVMODE_DAYLIGHT:
      ...
      if (!fldP) {
        ...
        if (tsFldP->isFloating()) return false; // floating -> no time zone
        // - get context
      ...
      if (aConvDefP->convmode==CONVMODE_TZID) {
        ...
        if (!TCTX_IS_UTC(tctx) && !TCTX_IS_UNKNOWN(tctx) && !TCTX_IS_DATEONLY(tctx)) {
          ...
          if (!TimeZoneContextToName(tctx, aString, getSessionZones())) return false; // cannot get name/ID
...
)
{
  ...
  char c;
  sInt16 n=0,llen=0;
  sInt16 lastlwsp=-1; // no linear white space found so far
  bool explf;
            ...
            if (aConvDefP->enumdefs) {
              ...
              if (enumP) {
                ...
                if (enumP->enummode==enm_ignore)
                  ...
                  sInt16 n=TCFG_SIZE(enumP->enumval);
  ...
  if (dolevel) {
    ...
    if (aProfileP->profileMode==profm_vtimezones) {
      ...
      for (TTCtxSet::iterator pos=fUsedTCtxSet.begin(); pos!=fUsedTCtxSet.end(); pos++) {
        ...
        // - generate raw string
        internalToVTIMEZONE(*pos, val, getSessionZones());
        size_t i,n = 0;
        while (val.size()>n) {
          i = val.find('\n',n); // next line end
          if (i==string::npos) i=val.size();
          if (i-n>1) {
            // more thank one char = not only a trailing line end
            s.assign(val,n,i-n);
        ...
        if (enumP) {
          ...
          if (enumP->enummode==enm_ignore)
            ...
            if (enumP->enummode==enm_prefix) {
              ...
              sInt16 n=TCFG_SIZE(enumP->enumtext);
...
)
{
  ...
  do {
    ...
    if (strucmp(propname,"BEGIN",n)==0) {
      ...
      if (*p==';') while (*p && *p!=':') p++;
      // - isolate value
      sInt16 l=0; const char *lnam=p+1;

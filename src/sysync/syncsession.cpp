  ...
  fSyncAppBaseP(aSyncAppBaseP) // link to owning base (dispatcher/clienbase)
{
  ...
  //       session zones
  fSessionZones.CopyCustomTZFrom(*(fSyncAppBaseP->getAppZones()));
...
void TSyncSession::AbortCommandProcessing(uInt16 aStatusCode)
{
...
void TSyncSession::SuspendSession(uInt16 aReason)
{
...
void TSyncSession::AbortSession(uInt16 aStatusCode, bool aLocalProblem, uInt16 aReason)
{
...
void TSyncSession::DatastoreFailed(uInt16 aStatusCode, bool aLocalProblem)
{
  fFailedDatastores++;
...
localstatus TSyncSession::checkRemoteSpecifics(SmlDevInfDevInfPtr_t aDevInfP)
{
  ...
  PDEBUGPRINTFX(DBG_HOT+DBG_REMOTEINFO,("- Max Request time [sec]    : %ld",fRequestMaxTime));
    ...
    if (smlPeekMessageBuffer(getSmlWorkspaceID(), aOutgoing, &data, &datasize)==SML_ERR_OK) {
      ...
      if (dbgOutP) {
        ...
        if (dbgOutP->openDbg(
          ...
          // write out the entire message
          dbgOutP->putRawData(data, datasize);
          // close the file
          dbgOutP->closeDbg();

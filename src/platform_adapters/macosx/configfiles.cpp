 ...
 *
 *  MacOSX Platform adaptor for accessing config files
 *
...
bool getPlatformString(TPlatformStringID aStringID, string &aString)
{
  ...
  switch (aStringID) {
    ...
    case pfs_appdata_path:
      ...
      getMacDir(aString,false);
      aString += "/Library/Application Support/" APPDATA_SUBDIR; // application specific subdir
      break;
    #endif
    case pfs_prefs_path:
      // general directory where all applications store data & prefs (for current user)
      getMacDir(aString,false);
      aString += "/Library/Application Support"; // all apps store their stuff here
      break;
    #endif // Mac OS X
    /*
    ...
    exedir:
      ...
      aString = %%% maybe show bundle path here
      break;
    */
    default:
      // unknown ID
...
bool getDeviceInfo(string &aURI, string &aName)
{
  #ifdef MOBOSX
  aURI = "iPhone_unknown_serial";
  aName = "iPhone with unknown serial";
  return false; // not really unique!
  #else
  char     szHostname[100];
  ...
  if (gethostname( szHostname, sizeof( szHostname ))!=0) {
    ...
    if (pHostEnt)
      aName=pHostEnt->h_name; // DNS name of machine
    else
      aName=szHostname; // just name of machine
  }
  // generate URI from name
  aURI="macosx:"; // %%% SCTS does not like http:// here, so we take os:xxxx
  // always use fully qualified name as ID, as this is most unique
  aURI+=aName;

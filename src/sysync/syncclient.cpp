...
localstatus TSyncClient::processAnswer(void)
{
  ...
  //%%%disturbs SyncML_Incoming and SyncML_Outgoing blocks: PDEBUGBLOCKDESC("ProcessAnswer","Processing SyncML Answer from Server");
  fIgnoreMsgErrs=false;
  err=smlProcessData(
    myInstance,
    SML_ALL_COMMANDS
  );
  //%%%disturbs SyncML_Incoming and SyncML_Outgoing blocks: PDEBUGENDBLOCK("ProcessAnswer");
  if (err && !fIgnoreMsgErrs) {
